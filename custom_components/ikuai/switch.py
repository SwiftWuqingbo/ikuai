...
async def async_setup_entry(hass, config_entry, async_add_entities):
    ...
    if coordinator.data.get("port_control"):
        listportdata = coordinator.data.get("port_control")
        if isinstance(listportdata, list):
            _LOGGER.debug(listportdata)
            for port in listportdata:
                _LOGGER.debug(port)
                switchsport.append(IKUAISwitchport(hass, coordinator, host, username, passwd, pas, port["id"]))                
            async_add_entities(switchsport, False)
...
class IKUAISwitchport(SwitchEntity):
    _attr_has_entity_name = True
    def __init__(self, hass, coordinator, host, username, passwd, pas, portid):
        """Initialize."""
        super().__init__()
        self.coordinator = coordinator        
        self._state = None
        ...
        listportswitch = coordinator.data.get("port_control")
        if isinstance(listportswitch, list):
            #_LOGGER.debug(listvmdata)
            for portswitch in listportswitch:
                if portswitch["id"] == portid:
                    _LOGGER.debug(portswitch)
                    self._wan_port = portswitch["wan_port"]
                    self._lan_port = portswitch["lan_port"]
                    self._wanport = str(self._wan_port).replace(",","")
                    self._lanport = str(self._lan_port).replace(",","")
                    if portswitch.get("comment"):
                        self._name = "Port_control_("+portswitch["comment"]+")"
                    else:
                        self._name = "Port_control_(未备注)"                    
                    self._is_on = portswitch["enabled"] == "yes"
                    self._state = "on" if self._is_on == True else "off"
                    self._querytime = self.coordinator.data["querytime"]
    ...
    @property
    def unique_id(self):
        return f"{DOMAIN}_switch_{self.coordinator.host}_{self._wanport}"
    ...
    async def async_turn_on(self, **kwargs):
        ...
        self._is_on = True
        self._change = False
        port_json_body = {"func_name":"dnat","action":"up","param":{"id":str(self._portid)}}
        await self._port_control_switch(port_json_body) 
    ...
    async def async_turn_off(self, **kwargs):
        ...
        self._is_on = False
        self._change = False
        port_json_body = {"func_name":"dnat","action":"down","param":{"id":str(self._portid)}}
        await self._port_control_switch(port_json_body)
    ...
    async def async_update(self):
        """Update entity."""
        #await self.coordinator.async_request_refresh()
        listportswitch = self.coordinator.data.get("port_control")
        if isinstance(listportswitch, list):
            #_LOGGER.debug(listvmdata)
            for portswitch in listportswitch:
                if portswitch["id"] == self._portid:
                    _LOGGER.debug(portswitch)
                    if self._change == True:
                        self._is_on = portswitch["enabled"] == "yes"
                        self._state = "on" if self._is_on == True else "off"
    ...
    async def _port_control_switch(self, action_body): 
        if self._allow_login == True:            
            sess_key = await self.get_access_token()          
            header = {
            'Cookie': 'Cookie: username=admin; login=1; sess_key='+sess_key,
